...
import { useSpecializations } from "@/hooks/use-specialization";
import {
	MultiSelect,
	MultiSelectContent,
	MultiSelectGroup,
...
export function RegisterSpecializations() {
	// Get store values
	const storeSpecializations = useOnboardingStore(
		(state) => state.specializations,
	);
	const setStoreSpecializations = useOnboardingStore(
	...
	// No data state
	if (!LEGAL_SPECIALIZATIONS || LEGAL_SPECIALIZATIONS.length === 0) {
		return (
			<div className="space-y-4">
				<div className="flex items-center justify-center p-8">
					<p className="text-sm text-muted-foreground">
