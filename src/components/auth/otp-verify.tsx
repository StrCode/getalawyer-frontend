...
import { Loading01Icon, Loading02Icon, Mail01Icon, MailOpen01Icon, PhoneCheckIcon, Refresh01Icon } from "@hugeicons/core-free-icons"
    ...
    >
      {cooldown > 0 ? (
        ...
        <span className="flex items-center gap-1">
          <HugeiconsIcon icon={Refresh01Icon} aria-hidden="true" className="size-3" />
          Resend code
        </span>
      )}
    </button>
...
function VerifyButton({
  ...
  return (
    ...
    >
      {isLoading ? (
        <>
          <HugeiconsIcon icon={Loading01Icon} aria-hidden="true" className="size-4 animate-spin" />
          Verifying…
        </>
      ) : (
        "Verify code"
...
export default function AuthOTPVerify({
  ...
  return (
    <Card className={cn("w-full max-w-md shadow-xs", className)}>
      <CardHeader>
